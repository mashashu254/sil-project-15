...
const Quiz = () => {
  ...
  const generatePlan = () => {
    ...
    if (
      (responses.priority === "Courses" ||
        responses.priority === "Extracurriculars") &&
      ...
      responses.academic_interests.forEach((broadCategory) => {
        if (academicLinks[broadCategory]) {
          ...
          plan += `<ul>`;
          for (const [major, link] of Object.entries(
            academicLinks[broadCategory].majors
          )) {
    ...
    if (responses.varsity === "Yes") {
      ...
      plan += `<ul>${varsityResources.templates
        .map(
          (template) =>
            `<li><a href="${template}" target="_blank">Example Template Link</a></li>`
        )
        .join("")}</ul>`;
      ...
      responses.hobbies.forEach((hobbyCategory) => {
        if (extracurricularClubs[hobbyCategory]) {
          ...
          plan += `<ul>${extracurricularClubs[hobbyCategory]
            .map(
              (club) =>
                `<li><strong>${club.name}</strong>: ${club.description}</li>`
            )
            .join("")}</ul>`;
        }
      });
    ...
    if (responses.hobbies && responses.hobbies.length > 0) {
      ...
      uniqueHobbies.forEach((hobbyCategory) => {
        if (extracurricularClubs[hobbyCategory]) {
          ...
          const shuffledClubs = extracurricularClubs[hobbyCategory].sort(
            () => 0.5 - Math.random()
          );
    ...
    <div className="container">
      ...
      {showAdvice && (
        <div className="advice-section">
          ...
          <div
            className="advice"
            dangerouslySetInnerHTML={{ __html: adviceOutput }}
          />
        </div>
      )}
    </div>
